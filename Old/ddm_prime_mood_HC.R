...
df <- read.csv(file = "ddm_morph_HC_patients-Alexane.csv") %>%
    rename(
        rt = RT, response = Response, mood = PC1,
...
priors <- c(
    ...
    # priors for the SD of the varying effects
...
get_init_value <- function (x) list(
    Intercept_ndt = rnorm(n = 1, mean = x, sd = epsilon)
    # Intercept = 0.01,
    # Intercept_bs = 0.01,
    # Intercept_bias = 0.01
    )
...
# retrieving the model estimates
summary(fit_wiener)
