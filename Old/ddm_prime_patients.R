...
df <- read.table(file = "ddm_patients.csv") %>%
  rename(rt = RT, response = Response, block = Block, prime_emotion = Prime, mood = PC1, participant = ID, group = Group.x, target = Target) %>%
  filter(rt > 0.1 & rt < 3) %>%
  mutate(block = factor(block)) %>%
  mutate(group = factor(group))

# computing RT mean and summary per condition
df %>% summarise(mean = mean(rt), sd = sd(rt), .by = c(block, prime_emotion,target) )

# checking RT distribution and quantiles
hist(df$rt, breaks = "FD")
quantile(df$rt, probs = seq(0, 1, 0.01) )

# plotting distributions of RTs per condition
df %>%
  ggplot(aes(x = rt, colour = prime_emotion, fill = prime_emotion) ) +
  geom_density(alpha = 0.4) +
  facet_wrap(block~target) +
...
df <- read.table(file = "ddm_patients.csv") %>%
  # filter(participant %in% 1:2) %>%
  rename(rt = RT, response = Response, block = Block, prime_emotion = Prime, mood = PC1, participant = ID, group = Group.x, target = Target) %>%
  filter(rt > 0.1 & rt < 3) %>%
  mutate(block = factor(block))

# only keep target = morph
df <- df[df$target == 'Morph',]

# defining the model formula (one "linear model" per parameter)
formula <- brmsformula(
  # drift rate (delta)
  rt | dec(response) ~ 1 + group * block * prime_emotion + (1 + block * prime_emotion | participant),
  # boundary separation parameter (alpha)
  bs ~ 1 + group * block * prime_emotion + (1 + block * prime_emotion| participant),
  # non-decision time (tau)
  ndt ~ 1 + group * block * prime_emotion + (1 + block * prime_emotion| participant),
  # starting point or bias (beta)
  bias ~ 1 + group * block * prime_emotion + (1 + block * prime_emotion| participant)
)
...
fit_wiener <- brm(
  ...
  file = "models/ddm_morph_patients.rds",
