...
setwd('/neurospin/b37t/DDM')

### importing the data 
df <- read.csv(file = "ddm_morph_HC_patients-Alexane.csv") %>%
  rename(rt = RT, response = Response, mood = PC1, participant = ID, group = Group) %>%
  filter(rt > 0.1 & rt < 3) %>%
  filter(group!='HC') %>%
  filter(Prime=='Control')
df$X <- NULL
...
formula <- brmsformula(
  # drift rate (delta)
  rt | dec(response) ~ 1 + mood * group + (1 | participant),
  # boundary separation parameter (alpha)
  bs ~ 1 + mood * group + (1 | participant),
  # non-decision time (tau)
  ndt ~ 1 + mood * group + (1 | participant),
  # starting point or bias (beta)
  bias ~ 1 + mood * group + (1 | participant))
